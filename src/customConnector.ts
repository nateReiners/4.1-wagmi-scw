...
import type { CoinbaseWalletProvider as CBW_Provider, CoinbaseWalletSDK as CBW_SDK } from '@coinbase/wallet-sdk';
import {
...
export type CoinbaseWalletParameters<version extends Version = '3'> = version extends '4'
  ? Evaluate<
      {
...
): version extends '4' ? ReturnType<typeof version4> : ReturnType<typeof version3> {
  if (parameters.version === '3' || parameters.headlessMode) return version3(parameters as Version3Parameters) as any;
...
function version4(parameters: Version4Parameters) {
  ...
  return createConnector<Provider, Properties>((config) => ({
    ...
    async getProvider() {
      if (!walletProvider) {
        console.log('no wallet provider detected, creating one...');
      ...
      console.log('returning wallet provider', walletProvider);
