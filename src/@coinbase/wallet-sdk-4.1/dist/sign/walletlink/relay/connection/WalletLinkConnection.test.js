...
jest.spyOn(WalletLinkCipher_1.WalletLinkCipher.prototype, 'decrypt').mockImplementation(decryptMock);
describe('WalletLinkConnection', () => {
    const session = new WalletLinkSession_1.WalletLinkSession(new ScopedLocalStorage_1.ScopedLocalStorage('walletlink', 'test'));
    let connection;
    let listener;
    beforeEach(() => {
        jest.clearAllMocks();
    ...
    describe('handleSessionMetadataUpdated', () => {
        ...
        it('should call listner.metadataUpdated when WalletUsername updated', async () => {
            ...
            expect(listener_metadataUpdatedSpy).toHaveBeenCalledWith(constants_1.WALLET_USER_NAME_KEY, await decryptMock(newUsername));
        });
        it('should call listner.metadataUpdated when AppVersion updated', async () => {
            ...
            expect(listener_metadataUpdatedSpy).toHaveBeenCalledWith(constants_1.APP_VERSION_KEY, await decryptMock(newAppVersion));
        });
