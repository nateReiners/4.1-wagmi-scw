 ...
 */
function queueMessageEvent({ data, origin = new URL(constants_1.CB_KEYS_URL).origin, }) {
...
describe('Communicator', () => {
    ...
    beforeEach(() => {
        ...
        urlOrigin = new URL(constants_1.CB_KEYS_URL).origin;
        mockPopup = {
            postMessage: jest.fn(),
            close: jest.fn(),
            closed: false,
        ...
        web_1.openPopup.mockImplementation(() => mockPopup);
    });
    describe('onMessage', () => {
    ...
    describe('postRequestAndWaitForResponse', () => {
        it('should post a message to the popup window and wait for response', async () => {
            const mockRequest = { id: 'mock-request-id-1-2', data: {} };
            ...
            expect(mockPopup.postMessage).toHaveBeenNthCalledWith(1, {
                data: {
    ...
    describe('postMessage', () => {
        it('should post a response to the popup window', async () => {
            const mockResponse = { requestId: 'mock-request-id-1-2', data: {} };
            queueMessageEvent(popupLoadedMessage);
    ...
    describe('waitForPopupLoaded', () => {
        it('should open a popup window and finish handshake', async () => {
            ...
            expect(web_1.openPopup).toHaveBeenCalledWith(new URL(constants_1.CB_KEYS_URL));
            expect(mockPopup.postMessage).toHaveBeenNthCalledWith(1, {
                data: {
                    version: version_1.LIB_VERSION,
                },
        ...
        it('should re-focus and return the existing popup window if one is already open.', async () => {
            ...
            web_1.openPopup.mockImplementationOnce(() => mockPopup);
            queueMessageEvent(popupLoadedMessage);
            await communicator.waitForPopupLoaded();
            expect(mockPopup.focus).toHaveBeenCalledTimes(1);
            expect(web_1.openPopup).toHaveBeenCalledTimes(1);
        });
        it('should open a popup window if an existing one is defined but closed', async () => {
            ...
            queueMessageEvent(popupLoadedMessage);
            await communicator.waitForPopupLoaded();
            expect(web_1.openPopup).toHaveBeenCalledTimes(2);
        });
    });
});
